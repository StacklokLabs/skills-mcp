...
- **Tools** - mirror the `Skill` tool pattern used natively by Claude Code, Roo Code, Cline, and Continue, giving universal tool-calling compatibility:
  - `list_skills` - Tier 1 catalog. The tool's own description embeds the current `<available_skills>` list (name + short description), matching Claude Code's pattern of surfacing the catalog in the description so a model knows what exists without a separate call. The embedded list is built against a ~1.9KB byte budget (clients such as Claude Code truncate tool descriptions at ~2KB): up to 10 full name+description entries, then a names-only "Also available: ..." overflow line, then a bare count. Live trials showed only full entries drive unprompted uptake — both a count-only marker and the names-only line failed to make a model page past the cap for a matching task (0/2 each), so catalog order determines which skills can fire on their own. The names line remains so a model that does read the description sees the complete inventory. The tool also carries the `anthropic/alwaysLoad` meta flag: clients with MCP tool search (Claude Code's default) defer tool descriptions out of context, and live trials showed agents never discover the server on natural prompts without this exemption (0/6 baseline); with the flag, unprompted uptake was 4/4. See [Agent uptake](../agent-uptake.md) for the full experiment history and operator guidance.
...
The server also declares MCP **instructions** that direct clients to the `list_skills` → `get_skill` → `get_skill_resource` workflow. The instructions are written as imperative trigger text rather than a capability statement: they name concrete trigger tasks (commit messages, release notes, PR descriptions), tell the model to check the catalog "even if you already know how to do the task", and explicitly state that these skills are separate from any built-in skills feature of the client. The last point matters in practice: "skills" is native Claude Code vocabulary, and before the disambiguation was added, nudges like "check what skills are available to you" routed to the client's built-in skill list and the MCP server was ignored. All four tools declare `readOnlyHint`/`idempotentHint`/`openWorldHint=false` annotations, which clients use to relax permission handling and parallelize calls.
